...
`gta11` is a browser 3D game — originally the "Sketchbook" engine (Three.js + Cannon.js physics: characters, vehicles, scenarios) — being migrated from Webpack to pnpm + Vite + React + TypeScript + React Three Fiber (R3F). This worktree (branch `gta11-r3f-vite`) is that migration in progress.
